...
import y3.common as common
import y3.backbone as backbone
...
def compute_loss(pred, conv, label, bboxes, i=0):
    ...
    #iou = tf.expand_dims(bbox_iou(pred_xywh, label_xywh), axis=-1)
    ...
    tp = giou[giou>.5].numpy().size   
    fp = giou[ (giou<.5) & (giou>.3) ].numpy().size
    al = respond_bbox[respond_bbox==1].size   
    #fn = al - ( tp + fp )
    fn = al - ( tp  )


    #print('ALL: {}'.format(al))
    #print('CLASS_PROBS: {} '.format(conv_raw_prob.shape))
    #print('CLASS_PROBS_NONZEROS: {} '.format(np.max(conv_raw_prob)))
    #print('GIOUS: {}'.format(giou.shape))
    #print('CONF_PROB: {}'.format(pred_conf.shape))
    #print('CONF_PROB_POSITIV: {}'.format(pred_conf[pred_conf>.3].numpy().size))
    ...
    return giou_loss, conf_loss, prob_loss, tp, fp, fn, al 
